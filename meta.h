...
template <unsigned int T1, unsigned int... Tn>
class LISTV<T1, Tn...>
{
protected:
    struct Head {
        static const unsigned int value = T1;
    };

    typedef LISTV<Tn...> Tail;

public:
    enum { Length = Tail::Length + 1 };

    template <unsigned int Index, unsigned int Current = 0, bool Stop = (Index == Current)>
    struct Get
    {
        static const unsigned int result = Tail::template Get<Index, Current + 1>::result;
    };

    template <unsigned int Index, unsigned int Current>
    struct Get<Index, Current, true>
    {
        static const unsigned int result = Head::value;
    };
};
...
public:
    ...
    template <unsigned int Index, unsigned int Current = 0>
    struct Get
    {
        static const unsigned int result = 0; // You can provide a default value or throw an exception here
    };
};
*/
...
struct LISTV<T1, Tn...>
{
    static constexpr unsigned int Length = sizeof...(Tn) + 1;

    template <unsigned int Index>
    static constexpr unsigned int Get()
    {
        //static_assert(Index < Length, "Index out of bounds");
        return Index == 0 ? T1 : LISTV<Tn...>::template Get<Index - 1>();
    }
};
...
struct LISTV<>
{
    ...
    static constexpr unsigned int Get()
    {
        ...
        return 0; // You can provide a default value or throw an exception here
    }
};
...
struct EFOR{
    template<class MultiUnitSmartData>
    static void exec(MultiUnitSmartData & msd)
    {
...
void SERIALIZE(char * buf, int index, const T & a) {
    ...
    std::cout<< "wrote in buffer final" << std::endl;
    std::cout << "printing the buffer " << buf << std::endl;
    std::cout << "printing the value of a" << a._unit << std::endl;
}
...
void SERIALIZE(char * buf, int index, const T & a, const Tn & ... an) {
    __builtin_memcpy(&buf[index], &a, sizeof(T));
    std::cout<< "sizeof T: " << sizeof(T) << std::endl;
    std::cout<< "wrote in buffer at"<< &buf << std::endl;
...
void DESERIALIZE(char * buf, int index, T && a) {
    ...
    std::cout<< "sizeof T: " << sizeof(T) << std::endl;
    std::cout<< "wrote in buffer2 final" << std::endl;
    std::cout << "printing the buffer2 " << buf << std::endl;
    std::cout << "printing the value of a" << a._unit << std::endl;
}
...
void DESERIALIZE(char * buf, int index, T && a, Tn && ... an) {
    __builtin_memcpy(&a, &buf[index], sizeof(T));
    std::cout<< "sizeof T: " << sizeof(T) << std::endl;
    std::cout<< "wrote in buffer2" << std::endl;
    std::cout << "printing the buffer2 " << buf << std::endl;
    std::cout << "printing the value of a" << a._unit << std::endl;
