...
public:
    ...
    void fill(unsigned int s, Tn ... an) {
        _size = s;
        Data data(an ...);
        memcpy(reinterpret_cast<unsigned char*>(this), reinterpret_cast<unsigned char*>(&data), s);
    }
...
public:
    Dynamic_Circular_Buffer(unsigned int capacity) : _capacity(capacity), _size(0), _head(0), _tail(0) { _data = new Object_Type[_capacity];}
    Dynamic_Circular_Buffer(const void * data, unsigned int size, unsigned int capacity) : _capacity(capacity) { _data = new Object_Type[_capacity]; copy_and_pad(data, size); }
    template <typename U>
    Dynamic_Circular_Buffer(const U & o, unsigned int capacity) : _capacity(capacity) { _data = new Object_Type[_capacity]; copy_and_pad(&o, sizeof(U)); }
